...
// Handle image file upload -> store BLOB directly in DB
$imageBlob = null;
if (isset($_FILES["image_file"]) && $_FILES["image_file"]["error"] === UPLOAD_ERR_OK) {
    ...
    if (!in_array($ext, $allowed)) {
        ...
        $imageBlob = file_get_contents($_FILES["image_file"]["tmp_name"]);
        if ($imageBlob === false) { $msg = "Failed to read image."; $mt = "error"; }
    }
}

if($msg!==""){/* skip insert */}else
if($nm===""||$pr<=0){$msg="Name and valid price required.";$mt="error";}else{mysqli_begin_transaction($conn);try{
// Insert with BLOB: use send_long_data for the image column
$s=mysqli_prepare($conn,"INSERT INTO Furniture(furniture_name,description,image,price,stock_quantity) VALUES(?,?,?,?,?)");
$nullVal = null;
mysqli_stmt_bind_param($s,"ssbdi",$nm,$dc,$nullVal,$pr,$sq);
mysqli_stmt_send_long_data($s, 2, $imageBlob !== null ? $imageBlob : "");
