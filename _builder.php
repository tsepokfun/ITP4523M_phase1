...
if($msg!==""){}else
if($nm===""||$pr<=0){$msg="Name and valid price required.";$mt="error";}else{mysqli_begin_transaction($conn);try{$s=mysqli_prepare($conn,"INSERT INTO Furniture(furniture_name,description,price,stock_quantity) VALUES(?,?,?,?)");mysqli_stmt_bind_param($s,"ssdi",$nm,$dc,$pr,$sq);if(!mysqli_stmt_execute($s))throw new Exception("Insert failed");$nf=mysqli_insert_id($conn);mysqli_stmt_close($s);
