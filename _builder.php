...
$sql="SELECT f.furniture_id,f.furniture_name,f.image,f.price,COUNT(o.order_id) as oc,COALESCE(SUM(o.order_quantity),0) as tq,COALESCE(SUM(o.total_amount),0) as ts FROM Furniture f LEFT JOIN Orders o ON f.furniture_id=o.furniture_id GROUP BY f.furniture_id ORDER BY ts DESC";
...
if($_SERVER["REQUEST_METHOD"]==="POST"){$nm=trim(isset($_POST["furniture_name"])?$_POST["furniture_name"]:"");$dc=trim(isset($_POST["description"])?$_POST["description"]:"");$im=trim(isset($_POST["image"])?$_POST["image"]:"");$pr=(float)(isset($_POST["price"])?$_POST["price"]:0);$sq=(int)(isset($_POST["stock_quantity"])?$_POST["stock_quantity"]:0);$mi=isset($_POST["material_id"])?$_POST["material_id"]:array();$mq=isset($_POST["material_quantity"])?$_POST["material_quantity"]:array();
if($nm===""||$pr<=0){$msg="Name and valid price required.";$mt="error";}else{mysqli_begin_transaction($conn);try{$s=mysqli_prepare($conn,"INSERT INTO Furniture(furniture_name,description,image,price,stock_quantity) VALUES(?,?,?,?,?)");mysqli_stmt_bind_param($s,"sssdi",$nm,$dc,$im,$pr,$sq);if(!mysqli_stmt_execute($s))throw new Exception("Insert failed");$nf=mysqli_insert_id($conn);mysqli_stmt_close($s);
...
mysqli_commit($conn);$msg="Furniture created (ID: $nf).";$mt="success";}catch(Exception $e){mysqli_rollback($conn);$msg=$e->getMessage();$mt="error";}}}mysqli_close($conn);
...
<div class="fc"><form method="post" id="ff"><div class="fr"><div><label>Furniture Name *</label><input type="text" name="furniture_name" required></div><div><label>Image File</label><input type="text" name="image" placeholder="e.g. oak_table.jpg"></div></div>
